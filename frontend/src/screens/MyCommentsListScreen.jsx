...
import { usePaginationQuery } from "../redux/slices/usersApiSlice"
import { useCountAllCommentsQuery } from "../redux/slices/commentApiSlice"

const MyCommentsListScreen = () => {
  ...
  const { commentCount } = useCountAllCommentsQuery()
  const { comments } = usePaginationQuery({ page: page, size: commentsPerPage })

  var totalPages = commentCount / commentsPerPage;

  const renderList = () => {
    if (comments) {
      return comments.map((comment) => <CommentCard key={comment.id} body={comment} />)
    }
